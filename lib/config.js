...
/** 데이터베이스 설정 (PostgreSQL) - DATABASE_URL > POSTGRES_* > DB_* */
export const DATABASE_URL       = process.env.DATABASE_URL || "";

let _databaseUrl = null;
if (DATABASE_URL) {
  try {
    _databaseUrl = new URL(DATABASE_URL);
  } catch {
    _databaseUrl = null;
  }
}

export const DB_HOST            = _databaseUrl?.hostname
...
export const DB_NAME            = (_databaseUrl?.pathname || "").replace(/^\//, "")
  || process.env.POSTGRES_DB
  || process.env.DB_NAME
  || "";
export const DB_USER            = decodeURIComponent(_databaseUrl?.username || "")
  || process.env.POSTGRES_USER
  || process.env.DB_USER
  || "";
export const DB_PASSWORD        = decodeURIComponent(_databaseUrl?.password || "")
