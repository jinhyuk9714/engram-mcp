<p align="center">
  <img src="assets/images/engram_mcp_logo_transparent.png" width="400" alt="Engram MCP Logo">
</p>
...
Engram MCP는 이 문제를 해결하는 용도로 만들어졌음.
...
```bash
# 제일 간단한 사용법: stdio MCP 서버를 바로 실행
...
`DATABASE_URL`이나 `POSTGRES_*`가 없으면 macOS/Linux에서는 Docker 기반 로컬 PostgreSQL을 자동으로 준비한다. 직접 DB를 붙이고 싶으면 `DATABASE_URL`을 주면 된다.

MCP 클라이언트 설정은 이렇게 최소화할 수 있다.
...
## 사용 팁: 시작할 때 context 먼저

MCP 서버가 `initialize.instructions`에서 기억 도구 사용을 권장하지만, 세션 시작 규칙을 프로젝트 지침에 적어두면 훨씬 안정적이다. `AGENTS.md`나 클라이언트 프로젝트 지침에 아래처럼 적어두면 된다.
...
한 줄 `npx` 설정과 이 규칙을 같이 쓰면 세션마다 처음 보는 사람처럼 행동하는 현상이 훨씬 줄어든다.
...
1. **비용 절약.** 다른 서비스(Mem0 등)는 기억을 찾을 때마다 매번 OpenAI 임베딩 API를 호출함. Engram MCP는 키워드로 바로 찾을 수 있으면 API 호출 없이 끝냄. 기억이 수백 개 쌓여도 API 비용이 비례해서 늘지 않고 더 효율적.
...
맞음. Engram MCP도 `remember` 시 해시 충돌이면 merge함 (`merged: true` 반환). 거기에 더해서 지금은 `remember` 시점에 pgvector로 같은 주제 유사 파편을 직접 찾아서 `related`, `resolved_by`, `superseded_by` 같은 관계를 자동으로 걸어줌. 중복 정리는 consolidate에서 배치로 처리하고, 관계 생성은 저장 시점에 바로 하는 하이브리드 방식. Mem0가 저장할 때마다 실시간으로 전부 임베딩 비교하는 거랑 다르게, Engram MCP는 저장은 빠르게 하되 유사도 기반 관계만 즉시 걸고 무거운 정리는 나중에 하는 구조.
